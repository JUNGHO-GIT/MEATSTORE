		...
		<section class="section-1">
			<div class="jumbotron d-flex align-items-center">
				...
				<div class="container-fluid content">
					...
					<h2>
						<button class="btn btn-jungho" onclick="window.location.href='${ctxpath}/board/writeForm.do'" data-aos="fade-up" data-aos-delay="300">글쓰기 </button>
					</h2>
				</div>
			</div>
		</section>
								...
								<tbody>
									<c:forEach var="dto" items="${list}">
										<tr>
											...
											<td>
												<a class="btn btn-jungho" href="${ctxpath}/board/content.do?num=${dto.num}&pageNum=${currentPage}"> + 더보기 </a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
							...
							<h6>
								<c:if test="${count > 0}">
									...
									<c:if test="${startPage > 5}">
										<button class="btn btn-jungho" onclick="window.location.href='${ctxpath}/board/list.do?pageNum=${startPage - 5}'">
											<i class="fa fa-angle-left" aria-hidden="true"></i>
										</button>
									</c:if>
									<c:forEach var="i" begin="${startPage}" end="${endPage}">
										<button class="btn btn-jungho" onclick="window.location.href='${ctxpath}/board/list.do?pageNum=${i}'"> &nbsp; ${i} &nbsp; </button>
									</c:forEach>
									<c:if test="${endPage < pageCount}">
										<button class="btn btn-jungho" onclick="window.location.href='${ctxpath}/board/list.do?pageNum=${startPage + 5}'">
